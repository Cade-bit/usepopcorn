...
function MovieListContent({ movies, onSelectMovie }) {
  return (
...
function MovieDetails({ selectedId, onCloseMovie, onAddWatched, watched }) {
  ...
  useEffect(
    // only add 1 to the counter when the user creates a rating
    function () {
          ...
          <section>
            <div className="rating">
              {!isWatched ? (
                ...
                <p>
                  You rated this movie already!! Rated: {watchedUserRating}
                  <span>⭐</span>
                </p>
              )}
            </div>
...
function WatchedSummaryHeader({ watched }) {
...
function WatchedSummaryContent({ watched, onDeleteWatched }) {
  return (
    <ul className="list">
      {watched.map((movie) => (
        <WatchedMovie
